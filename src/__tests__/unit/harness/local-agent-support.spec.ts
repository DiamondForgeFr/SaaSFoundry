...
describe('checkout-local agent support', () => {
  ...
  it('recovers owned generated output after final private-state persistence fails', async () => {
    ...
    await expect(enable(['codex'])).rejects.toThrow('Injected final inventory failure')
    expect((await privateState()).agents).toEqual([])
    expect((await privateState()).fileHashes).toEqual({})
    expect((await privateState()).pendingFileHashes['AGENTS.md']).toBeTruthy()
    jest.restoreAllMocks()
    const retry = await enable(['codex'])
